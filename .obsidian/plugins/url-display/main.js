...
var UrlDisplaySettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Deduplicate URLs").setDesc("If enabled, 2 same URLs in the note will only display once.").addToggle(
      (value) => value.setValue(this.plugin.settings.deduplicateUrls).onChange((value2) => {
        this.plugin.settings.deduplicateUrls = value2;
        this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Ignore file property").setDesc("If enabled, URLs in file properties will not be extracted and displayed.").addToggle(
    ...
    new import_obsidian.Setting(containerEl).setName("Use alias").setDesc("If enabled, the text in brackets ([]) will be used to display, otherwise the website title from URL metadata will be used.").addToggle(
      (value) => value.setValue(this.plugin.settings.useAlias).onChange((value2) => {
        this.plugin.settings.useAlias = value2;
        this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Show favicon").setDesc("If enabled, URL favicons will be shown in the pane.").addToggle(
      (value) => value.setValue(this.plugin.settings.showFavicon).onChange((value2) => {
        this.plugin.settings.showFavicon = value2;
        this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Show indicator icon").setDesc("If enabled, icons will be shown in the pane to identify URLs. This will only take effect when show favicon is disabled.").addToggle(
      (value) => value.setValue(this.plugin.settings.showIndicatorIcon).onChange((value2) => {
        this.plugin.settings.showIndicatorIcon = value2;
        this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Cache mode").setDesc("Choose cache mode for saving URL favicons. This will only take effect when show favicon is enable.").addDropdown((value) => {
      value.addOptions({ diskCache: "disk cache", memoryCache: "memory cache" }).setValue(this.plugin.settings.cacheMode).onChange((value2) => {
        this.plugin.settings.cacheMode = value2;
        this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerEl).setName("Notice mode").setDesc("Customize the type of notifications when URL parsing finishes.").addDropdown((value) => {
      value.addOptions({ none: "none", successful: "successful", failed: "failed", both: "both" }).setValue(this.plugin.settings.noticeMode).onChange((value2) => {
...
// src/views.ts
var UrlDisplayView = class extends import_obsidian2.ItemView {
  constructor(leaf, plugin, processor) {
    super(leaf);
    this.eventListeners = [];
    this.onHeaderMenu = (menu) => {
      menu.addItem((item) => {
        item.setTitle("Refresh list").setIcon("refresh-cw").onClick(async () => {
    ...
    this.updateList = (container) => {
      ...
      if (this.processor.activeNoteUrlParse) {
        this.processor.activeNoteUrlParse.forEach((currentUrl) => {
          ...
          const handler = (event) => this.handleMousedown(event);
          ...
          if (this.plugin.settings.showFavicon) {
            if (currentUrl.icon) {
              ...
              navUrlItem.appendChild((0, import_obsidian2.getIcon)("globe"));
            }
          ...
          const navUrlItemNavigation = (0, import_obsidian2.getIcon)("navigation");
...
var Parser = class {
  ...
  async parse(url) {
    ...
    const res = await (0, import_obsidian3.requestUrl)({ url: requestAPI });
  ...
  static async encodedIcon(icon) {
    const request = await (0, import_obsidian3.requestUrl)({ url: icon });
    const iconBase64 = "data:image/png;base64," + (0, import_obsidian3.arrayBufferToBase64)(request.arrayBuffer);
    return iconBase64;
  }
};
...
var markdownProcessor = class {
  constructor(plugin) {
    this.process = (0, import_obsidian4.debounce)(async (view) => {
      var _a, _b;
      this.initState();
      this.activeView = view;
      this.activeViewType = view == null ? void 0 : view.getViewType();
      if (this.activeView && SUPPORTED_VIEW_TYPE[this.activeViewType]) {
        ...
        if (!activeNoteUrl) {
          ...
          const currentMarkdownView = this.plugin.app.workspace.getActiveViewOfType(import_obsidian4.MarkdownView);
          if (currentMarkdownView) {
            if (((_a = currentMarkdownView.file) == null ? void 0 : _a.path) === ((_b = view == null ? void 0 : view.file) == null ? void 0 : _b.path)) {
              this.updateView();
            }
          } else {
            this.updateView();
    ...
    this.extractUrl = async (activeFile) => {
    ...
    this.parseUrl = async (activeView) => {
      ...
      if (this.plugin.settings.useAlias && !this.plugin.settings.showFavicon) {
        if (this.plugin.settings.noticeMode === "successful" || this.plugin.settings.noticeMode === "both") {
          new import_obsidian4.Notice("Successfully parsed all URLs \u{1F389}");
        }
        this.isParsing = false;
        return cleanedUrls;
      } else {
        ...
        if (failedCount === 0 && (this.plugin.settings.noticeMode === "successful" || this.plugin.settings.noticeMode === "both")) {
          ...
          new import_obsidian4.Notice(`Failed to parse ${failedCount} URLs \u{1F625}`);
        }
        this.isParsing = false;
        return cleanedUrls;
      }
    ...
    this.locateUrl = (content) => {
      ...
      while (match = URLREGEX.exec(content)) {
        ...
        const alias = ((_a = match[1]) == null ? void 0 : _a.replace(/(\*\*|__|~~|\*|==|`)/g, "").trim()) || "";
...
// src/main.ts
var UrlDisplayPlugin = class extends import_obsidian5.Plugin {
  async onload() {
    ...
    this.addRibbonIcon("external-link", "Open or close url display", (evt) => {
