...
__export(main_exports, {
  default: () => TickTickPlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian = require("obsidian");
var import_lodash = __toESM(require_lodash());
var client_id = "OcvIKsf0r001X1iDAS";
var callbackUrl = "https://ticktick.com/integrations/oauth/obsidian";
...
var TickTickPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    ...
    this.createTask = async (taskData) => {
      try {
        ...
        if (data) {
          new import_obsidian.Notice(`Add ${taskData.title}`);
        }
      } catch (error) {
        new import_obsidian.Notice("Add Failed");
        return new Promise((resolve, reject) => reject(error));
      }
    };
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: () => {
        if (this.checkUserLoginStatus()) {
          const title = (() => {
            var _a, _b, _c;
            const activeFile = (_a = this.app.workspace.activeEditor) == null ? void 0 : _a.file;
            const sel = (_c = (_b = this.app.workspace.activeEditor) == null ? void 0 : _b.editor) == null ? void 0 : _c.getSelection();
            if (activeFile) {
              return `[${sel || activeFile.name}](${this.getFileLink(activeFile)})`;
            }
            return "";
          })();
    ...
    this.addCommand({
      ...
      callback: async () => {
        if (this.checkUserLoginStatus()) {
          await this.fetchUserData();
          new import_obsidian.Notice("Data fetched successfully");
        }
      }
    });
...
var CreateTaskModal = class extends import_obsidian.Modal {
  constructor(app, plugin, taskData) {
    super(app);
    this.plugin = plugin;
  ...
  onOpen() {
    ...
    const titleComp = new import_obsidian.TextComponent(
      contentEl.createDiv({ cls: "modal-line" })
    ).setPlaceholder("Title").setValue(this.taskData.title).onChange((value) => {
      this.taskData.title = value;
    });
    titleComp.inputEl.style.flex = "auto";
    const contentComp = new import_obsidian.TextAreaComponent(
      contentEl.createDiv({ cls: "modal-line" })
    ).setPlaceholder("Content").setValue(this.taskData.content).onChange((value) => {
    ...
    const projectComp = new import_obsidian.DropdownComponent(projectLine).addOptions(
      this.plugin.settings.projects.filter((project) => !project.closed).sort((project) => project.sortOrder).reduce(
        (id2Name, project) => {
    ...
    ).setValue(this.taskData.projectId).onChange((value) => {
    ...
    const priorityComp = new import_obsidian.DropdownComponent(priorityLine).addOptions({
      0: "None",
      1: "Low",
      3: "Medium",
      5: "High"
    }).setValue(this.taskData.priority.toString()).onChange((value) => {
      this.taskData.priority = +value;
    });
    priorityComp.selectEl.style.flex = "auto";
    const submitComp = new import_obsidian.ButtonComponent(contentEl).setButtonText("Create").setCta().onClick(() => {
      if (this.taskData.title) {
        this.plugin.createTask(this.taskData);
        this.close();
      } else {
        new import_obsidian.Notice("Task title can't be empty");
      }
    });
...
var SettingTab = class extends import_obsidian.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.debounceTokenValidation = (0, import_lodash.debounce)(async (token) => {
  ...
  display() {
    ...
    tokenDesc.textContent = "This Plugin need your TickTick token to fetch the API, you can the token ";
    const tokenLink = containerEl.createEl("a");
    tokenLink.textContent = "here";
    tokenLink.href = url;
    tokenDesc.appendChild(tokenLink);
    new import_obsidian.Setting(containerEl).setName("API Token").setDesc(tokenDesc).addText(
      (text) => text.setValue(this.plugin.settings.token).onChange((token) => {
        this.debounceTokenValidation(token);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Fetch data background").setDesc(
      "Automatically fetches your project data in the background to ensure you have the latest data when creating a task"
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoFetchData).onChange(async (value) => {
